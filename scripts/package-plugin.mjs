...
export async function packagePlugin(args = process.argv.slice(2)) {
  ...
  try {
    await mkdir(path.join(stage, ".agents", "plugins"), { recursive: true }); await cp(path.join(root, ".agents", "plugins", "marketplace.json"), path.join(stage, ".agents", "plugins", "marketplace.json")); await cp(path.join(root, "plugins", "lgtm"), path.join(stage, "plugins", "lgtm"), { recursive: true, dereference: false });
